...
void tcp_send_files(int sock, FILE *fp, size_t size){
    ...
    print_progress((double)total_bytes_sent/(double)size, (float)size/1024, (float)total_bytes_sent/1024);
    green();
...
void tcp_recv_files(int sock, FILE *fp, size_t size) {
    ...
    // Verificacion del 100%
    print_progress((double)total_bytes_received/(double)size, (float)size/1024, (float)total_bytes_received/1024);
    green();
